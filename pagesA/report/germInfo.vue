			...
			<view class="mt20">
				...
				<view class="mt10 f-12">
					<view class="flex-box align-center info-title ptb-10 plr-5">
						...
						<text class="flex-item tc">菌落数</text>
						<text class="flex-item tc">格兰氏染色体</text>
						<text class="flex-item tc">培养结果</text>
					</view>
					<view v-for="item in info.itemsList" :key="item.id" class="flex-box align-center info-item ptb-10 plr-5">
						...
						<text class="flex-item tc">{{ item.colonyCount || '' }}</text>
						<text class="flex-item tc">{{ item.gramsRain || '' }}</text>
						<text class="flex-item tc">{{ item.cultureResult || '' }}</text>
					</view>
				</view>
			</view>
		</view>
...
<script>
import { getInspectionXJDetail } from '@/common/http/api.js';
export default {
	data() {
		return {
			loading: false,
