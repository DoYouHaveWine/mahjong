<template>
	<view>
		...
		<view class="toBePay-box" v-if="orderList.length">
			<view class="item" v-for="item in orderList" :key="item.orderId" @click="$navTo(`../order/details?orderId=${item.orderId}`)">
				<view class="title-box">
					...
					<text :style="{ color: item.status == 0 ? '#FF6262' : '#28D094' }">
						{{ constantMap.get('payStatusName')[item.status] || '' }}
					</text>
				</view>
				...
				<view class="content">
					...
					<view>
						...
						<text class="value" style="color: #ed8c53;">¥ {{ item.amount || '' }}</text>
					</view>
				</view>
		...
		<load-data
			v-if="!orderList.length || loading.isLoading"
...
<script>
import { getClinicOrderList, createWxOrder } from '@/common/http/api.js';
...
export default {
	...
	methods: {
		...
		async fetchOrderList() {
			...
			this.orderList = [];
			// this.form.recordId = 3478;
			try {
				const { data } = await getClinicOrderList(this.form);
						...
						.then(({ code, data }) => {
							if (code == 200) {
								uni.requestPayment({
									...
									success: res => {
										uni.navigateTo({ url: '/pagesA/payment/paySuccess' });
									},
									fail: err => {
		...
		setTimer() {
			this.timer = setInterval(() => {
				if (this.orderList) {
					this.orderList = this.orderList.map(element => {
						element.expireTime--;
						return element;
					});
				}
