<template>
	<view class="dropdown-item" v-if="active" @touchmove.stop.prevent="() => {}" @tap.stop.prevent="() => {}">
		<block v-if="!$slots.default && !$slots.$default">
			...
			>
				<view class="dropdown-item__options">
					<u-cell-group>
						<u-cell
 ...
 * @example <dropdown-item title="标题"></dropdown-item>
 */
export default {
	name: 'dropdownItem',
	props: {
	...
	computed: {
		// 监听props是否发生了变化，有些值需要传递给父组件dropdown，无法双向绑定
		propsChange() {
	...
	methods: {
		init() {
			// 获取父组件dropdown
			let parent = this.$u.$parent.call(this, 'dropdown');
			if (parent) {
				this.parent = parent;
